...
[![Tests](https://img.shields.io/badge/tests-858%20passing-brightgreen)](#testing)
...
| Song browser | `L` looks lyrics up (asking the catalogue about a track of THIS length) · `K` aligns them against the song's audio (needs the model from SETTINGS; `K` again cancels) · the **OPT** column says what a song still owes — `CHART`, `WORDS`, `BOTH` or `OK` — and sorting by it makes the list a to-do list · column headers **click to sort** (again = reverse) · `S` cycles sorting · `F` (or a typed `/`) opens the **fuzzy search** — every typed word must match a word of title, artist or genre (case, accents and apostrophes ignored, a typo per word forgiven), best match first; **hold `Q` one second** to leave it with the filter kept, `Esc` clears |
