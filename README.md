...
[![Tests](https://img.shields.io/badge/tests-1054%20passing-brightgreen)](#testing)
...
```bash
cargo test --workspace          # 1054 tests
```
...
like any other import. Nothing is fetched until you press the key, and
...
**AI SEARCH** (Settings, **off by default**) lets a model help that
search pick which recording to fetch. It is sent the **candidate
titles** — no audio, no lyrics, nothing about you — and answers with a
number. Where it runs is whatever your machine has: the **Claude Code
CLI** if it is installed (already signed in, so no key is stored
anywhere), otherwise an **API key** you put in Settings or export as
`ANTHROPIC_API_KEY`. With neither, the search still works: it ranks
recordings on their length and their own titles, which is what it does
