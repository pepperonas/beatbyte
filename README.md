...
[![Tests](https://img.shields.io/badge/tests-1020%20passing-brightgreen)](#testing)
...
*Hype running: double points, a held sustain beam
on the blue lane, the corner instrument panel (score well, multiplier
box, streak bulbs, the Hype tube with its star lit and turning) — on
the 3D stage with its moving heads, LED wall, full PA stacks and a
crowd of people dancing to the song. Every lane has its own gem shape,
so color is never the only signal.*

<p align="center">
  <img src="docs/media/beatbyte-round-style.png" alt="BeatByte round note style: a dark instrument neck with pale strings, black-ring strum notes and white-cap hammer-ons, a marked energy phrase, a band on the stage behind the neck, the crowd dial and hype tube" width="800"/>
</p>

*The same engine with round gems switched on: a dark instrument neck
with pale strings, strum notes with a black ring and hammer-ons with a
white cap, a held blue sustain with its pale core, a star-marked
energy phrase on the purple band, a band playing on the riser the
neck runs into under a warm key and a cool rim, and the crowd's dial
with the star-crowned Hype tube beside it. The note style is presentation only — the autopilot's
judgment counts prove it does not change.*
...
```bash
cargo test --workspace          # 1020 tests
```
