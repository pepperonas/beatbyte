...
[![Tests](https://img.shields.io/badge/tests-1042%20passing-brightgreen)](#testing)
...
- 🔎 **Add a song by name** — type it in the browser's search box and
  press `Y`: BeatByte looks the song up, judges the recordings it
...
| Song browser | `Y` searches the internet for whatever is typed in the search box and adds the song it finds (needs `yt-dlp`; SETTINGS → AI SEARCH lets a model help pick the recording) · `L` looks lyrics up (asking the catalogue about a track of THIS length) · `K` aligns them against the song's audio (needs the model from SETTINGS; `K` again cancels) · two marks say what the AI has been through: **LYRICS** is nothing (no words), a dim microphone (line stamps only) or a lit one with waves (every word placed), **CHART** is one dim bar (the import's own draft) or one lit rising bar per redesigned generation — independent of each other and both sortable, so the work floats to the top · column headers **click to sort** (again = reverse) · `S` cycles sorting · `F` (or a typed `/`) opens the **fuzzy search** — every typed word must match a word of title, artist or genre (case, accents and apostrophes ignored, a typo per word forgiven), best match first; **hold `Q` one second** to leave it with the filter kept, `Esc` clears |
...
```bash
cargo test --workspace          # 1042 tests
```
...
Pressing **Y** searches the internet for what you typed in the search
box. The **typed text** goes to `yt-dlp` on your machine, which asks
