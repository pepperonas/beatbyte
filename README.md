...
| Song browser | `D` opens a field to **add a song by name** — type it, `Enter` searches the internet and adds what it finds, `Esc` cancels (needs `yt-dlp`; SETTINGS → AI SEARCH lets a model help pick the recording) · `L` looks lyrics up (asking the catalogue about a track of THIS length) · `K` aligns them against the song's audio (needs the model from SETTINGS; `K` again cancels) · two marks say what the AI has been through: **LYRICS** is nothing (no words), a dim microphone (line stamps only) or a lit one with waves (every word placed), **CHART** is one dim bar (the import's own draft) or one lit rising bar per redesigned generation — independent of each other and both sortable, so the work floats to the top · column headers **click to sort** (again = reverse) · `S` cycles sorting · `F` (or a typed `/`) opens the **fuzzy search** — every typed word must match a word of title, artist or genre (case, accents and apostrophes ignored, a typo per word forgiven), best match first; **hold `Q` one second** to leave it with the filter kept, `Esc` clears |
...
```bash
cargo test --workspace          # 1079 tests
```
...
| `beatbyte-cli` | 23 | The play-history export (CSV quoting, UTC stamps, report filters), review analytics, the design dossier and the redesign rollout: section windowing, evidence thresholds, hash binding, the mastery veto, write instructions, carried difficulties, and the currency check comparing what a reader would get rather than what the generator produced |
