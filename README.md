...
[![Tests](https://img.shields.io/badge/tests-945%20passing-brightgreen)](#testing)
...
the 3D stage with its light rig, LED wall, PA stacks and crowd. Every
lane has its own gem shape, so color is never the only signal.*
...
neck runs into, and the crowd's dial with the star-crowned Hype tube
beside it. The note style is presentation only — the autopilot's
