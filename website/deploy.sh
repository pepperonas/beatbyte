#!/usr/bin/env bash
# Deploy the product page to beatbyte.celox.io (generated from templates/apps/product-page).
...
if [[ "${1:-}" == server ]]; then
  ...
  scp server/nginx/beatbyte.celox.io "$HOST:/etc/nginx/sites-available/beatbyte.celox.io"
  ssh "$HOST" "chmod 755 /usr/local/sbin/$SLUG-latest.py &&
    ln -sf /etc/nginx/sites-available/beatbyte.celox.io /etc/nginx/sites-enabled/ &&
