# BeatByte — product page. Generated from templates/apps/product-page.
# Live copy: /etc/nginx/sites-available/beatbyte.celox.io (files in sites-enabled/ are included inside http {},
...
map $http_accept $beatbyte_wants_markdown {
    default 0;
    "~*text/markdown" 1;
}

# /download follows the visitor's platform; first match wins, default = first target.
map $http_user_agent $beatbyte_target {
    default macos;
    "~*macintosh|mac os x" macos;
    "~*windows" windows;
    "~*^(?!.*android).*linux" linux-appimage;
}

server {
    listen 80;
    listen [::]:80;
    server_name beatbyte.celox.io beat-byte.celox.io;
    location /.well-known/acme-challenge/ { root /var/www/html; }
    location / { return 301 https://beatbyte.celox.io$request_uri; }
}

server {
    listen 443 ssl http2;
    listen [::]:443 ssl http2;
    server_name beat-byte.celox.io;
    ssl_certificate     /etc/letsencrypt/live/beatbyte.celox.io/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/beatbyte.celox.io/privkey.pem;
    include /etc/letsencrypt/options-ssl-nginx.conf;
    ssl_dhparam /etc/letsencrypt/ssl-dhparams.pem;
    return 301 https://beatbyte.celox.io$request_uri;
}

server {
    listen 443 ssl http2;
    listen [::]:443 ssl http2;
    server_name beatbyte.celox.io;
    ssl_certificate     /etc/letsencrypt/live/beatbyte.celox.io/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/beatbyte.celox.io/privkey.pem;
    include /etc/letsencrypt/options-ssl-nginx.conf;
    ssl_dhparam /etc/letsencrypt/ssl-dhparams.pem;

    root /var/www/beatbyte.celox.io;
    index index.html;
    charset utf-8;
    ...
    # /download and /download/<target>: written by beatbyte-latest.py (glob: absent file is fine).
    include /etc/nginx/beatbyte-download*.conf;
    ...
    location = / {
        if ($beatbyte_wants_markdown) { rewrite ^ /index.md last; }
    ...
    location = /.well-known/ard.json {
        alias /var/www/beatbyte.celox.io/.well-known/ai-catalog.json;
        default_type application/json;
        add_header Cache-Control "no-cache" always;
        add_header Access-Control-Allow-Origin "*" always;
        add_header X-Content-Type-Options "nosniff" always;
