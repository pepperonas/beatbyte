#!/usr/bin/env python3
"""Publish the newest BeatByte release next to its product page.

Generated from templates/apps/product-page. Runs on the VPS from beatbyte-latest.timer (every 15 min)
and writes, each file only when its content changed:
  ...
  /etc/nginx/beatbyte-download.conf
...
CONFIG = json.loads(r'''{
 "repo": "pepperonas/beatbyte",
 "branch": "main",
 "slug": "beatbyte",
 "domain": "beatbyte.celox.io",
 "webroot": "/var/www/beatbyte.celox.io",
 "nginx_include": "/etc/nginx/beatbyte-download.conf",
 "nginx_var": "beatbyte",
 ...
 "targets": [
  {
   ...
   "short": "macOS (Apple silicon)",
   "asset": "^BeatByte-[0-9][0-9.]*-aarch64-apple-darwin\\.dmg$",
  ...
  },
  {
   ...
   "short": "macOS (Intel)",
   "asset": "^BeatByte-[0-9][0-9.]*-x86_64-apple-darwin\\.dmg$",
   "requirement": "macOS 11+ · Intel"
  },
  {
   "id": "windows",
   "label": "Download for Windows",
   "short": "Windows",
   "asset": "^beatbyte-v[0-9][0-9.]*-x86_64-pc-windows-msvc\\.zip$",
   "requirement": "Windows 10/11 · 64-bit · portable zip"
  },
  {
   "id": "linux-appimage",
   "label": "Download AppImage",
   "short": "Linux AppImage",
   "asset": "^BeatByte-[0-9][0-9.]*-x86_64\\.AppImage$",
