# Product page — beatbyte.celox.io

The app's website: <https://beatbyte.celox.io>. Generated from
...
| `styles.css` | All styling. Colours are the app's own dark scheme (packaging/icon.png (neon cyan to magenta on near-black) and crates/beatbyte-game/src/palette.rs (BACKGROUND, SURFACE, TEXT)) as tokens on `:root`, from `site.json` → `theme`. |
...
| `server/beatbyte-latest.py` + `.service` / `.timer` | The timer (see below). Installed to `/usr/local/sbin/` and `/etc/systemd/system/`. |
| `server/nginx/beatbyte.celox.io` | Vendored vhost — the live file is `/etc/nginx/sites-available/beatbyte.celox.io`. |
...
`--delete`): `latest.json`, `changelog.md`, `ssi/`, `/etc/nginx/beatbyte-download.conf`.

## How "always the newest release" works

`beatbyte-latest.timer` runs `beatbyte-latest.py` every 15 minutes (and 2 minutes after boot). It asks the
...
| `/etc/nginx/beatbyte-download.conf` — `/download/<target>` → 302 to that asset; `/download` → the visitor's platform (a `map` on `User-Agent` in the vhost, first target by default) | The download button in the HTML and every link that should survive releases |
...
```bash
ssh root@69.62.121.168 'systemctl start beatbyte-latest.service; journalctl -u beatbyte-latest -n 3 -o cat'
# beatbyte-latest: v1.11.0 json=new ssi=new changelog=new nginx=reloaded
```
...
| What | Where | Notes |
|------|-------|-------|
| Newest release | `https://beatbyte.celox.io/download`, `/download/<target>` | 302 to the current GitHub release asset — `/download` for the visitor's platform. The download button's `href` in the HTML is `/download`; JavaScript picks the platform in the browser and swaps in the direct asset URL. |
...
```bash
curl -sI https://beatbyte.celox.io/ | grep -i -E '^link|^vary'
curl -s  -H 'Accept: text/markdown' https://beatbyte.celox.io/ | head -20
curl -s  https://beatbyte.celox.io/latest.json
```
...
- The language comes from `navigator.languages` (first of en/de/es/it/fr), is remembered in
  `localStorage` (`beatbyte-lang`) and switched with the flag menu (keyboard: arrows, Home/End, Enter,
...
./website/deploy.sh server   # also: beatbyte-latest.py, its units, the vhost, nginx reload
```
...
2. **Certificate:** put an HTTP-only vhost in place that serves `/.well-known/acme-challenge/` from
   ...
   `certbot certonly --webroot -w /var/www/html -d beatbyte.celox.io -d <alias> … --deploy-hook "systemctl reload nginx"`.
3. `mkdir -p /var/www/beatbyte.celox.io`, then `./website/deploy.sh server`.
...
echo | openssl s_client -connect beatbyte.celox.io:443 2>/dev/null | openssl x509 -noout -ext subjectAltName -enddate
ssh root@69.62.121.168 'systemctl list-timers beatbyte-latest.timer --no-pager'
```
