//! The song library: the bundled demo plus every valid chart found in
//! the `songs/` directory.
//!
...
pub enum SongSource {
    /// The synthesized in-memory demo.
    BuiltinDemo,
...
pub struct SongLibrary {
    /// All playable entries; the demo is always first.
...
pub fn scan_library(demo_chart: &ChartFile) -> SongLibrary {
    let mut entries = vec![SongEntry {
        title: demo_chart.song.title.clone(),
        artist: demo_chart.song.artist.clone(),
        bpm: demo_chart.song.bpm,
        duration_s: demo_chart.song.duration_s,
        difficulties: demo_chart.charts.iter().map(|c| c.difficulty).collect(),
    ...
    for chart_path in roots.iter().flat_map(|root| find_chart_files(root)) {
        match load_entry(&chart_path) {
            Ok(Some(entry)) => {
                ...
                // don't list it twice.
                if entry.title == demo_chart.song.title && entry.artist == demo_chart.song.artist {
                    continue;
                }
                entries.push(entry);
            }
    ...
    // Demo stays first; the rest sorts by title.
    entries[1..].sort_by_key(|entry| entry.title.to_lowercase());
    SongLibrary { entries }
}
