...
//!
//! Regenerates **hard + expert** from a fresh, deterministic reading
...
//!
//! It lives here rather than in the command line because the game
//! runs it too, from the song browser: one redesign, not two that
//! drift. The reading of the recording is INJECTED ([`Reading`]), so
//! this crate still knows nothing of `beatbyte-audio`.
    ...
    if let Some(grid) = merged.grid.as_ref() {
        for chart in &mut merged.charts {
            if !REDESIGNED.contains(&chart.difficulty) {
                grid.snap_notes(&mut chart.notes, crate::grid::SNAP_TOLERANCE_S);
            }
        }
    }
...
/// What the caller must tell this module about the audio.
///
/// The redesign needs a fresh reading of the recording, and reading
/// recordings is `beatbyte-audio`'s job — a crate this one does not
/// depend on and should not start depending on. So the caller brings
/// the reading and this module does the rest, which is what lets the
/// game and the CLI run the SAME redesign instead of two copies of
/// it that drift.
pub struct Reading {
    /// The fresh analysis of the recording.
    pub analysis: beatbyte_core::SongAnalysis,
    /// The timeline that analysis is on.
    pub trim: AudioTrim,
}

/// Redesign one song folder, in place, as its next version.
///
/// `read` supplies the fresh reading of the folder's audio; `note`
/// takes the diagnostics a command-line caller wants to print and a
...
pub fn redesign_folder(
    ...
    let Reading { analysis, trim } = read(&audio_path)?;
    active.retime(trim);
    ...
    crate::save_chart_file(&next_path, &merged)
