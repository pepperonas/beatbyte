...
- [x] **The arc is electric blue, with an aura and a surge** *(v0.18.49, 2026-09-25, user commission)* — every bolt segment and fork wears a wider electric-blue hull as a child (inherits pose and scale: zero per-frame cost) around a pale hot core; a cyan-blue aura of eight slabs per rail lies on the deck outside the neck (a short faint lip over it — the outer gems stay legible, measured +8 at most in the outer lane), fading through a baked gradient texture, its brightness a material-handle swap over a twelve-step ladder driven by a pure envelope that flares on the bolt's own flash steps and throbs at 1.6 Hz; a pure surge wave runs along the rail toward the horizon lifting bolt height, thickness, fork length and aura level. Calm: the aura at one level, the surge at half speed and a third of the lift. ⚠️ Two lessons, both in CLAUDE.md: a base alpha that scales with the level dims twice under additive blending (the first aura was invisible), and a vertex layout new to the scene compiles its pipeline on the first draw — a one-second stall the autopilot read as a clock teleport, twice, gone once the slab took the rig's textured unlit variant. *Verified: 1970 tests (+5), gate green; eight mutations of colour, surge direction and amplitude, envelope coupling and calm, intensity and gradient all caught; Hype frames photographed on "Heroes" at 39/41.5/44 s with `reduced_flashing` off and on, luma read and the frames looked at; uncapped with the window raised: 6.22–6.29 ms a frame during Hype against 6.17–6.26 on the baseline binary.*
- [x] **The phrase lands as a strike on its fret** *(v0.18.50, 2026-09-25, user commission)* — `gameplay/strike.rs`: armed off `PhraseCompleted` like the impulse (no star-power logic in a visual system), the fret looked up from the player's own track (`strike_targets`: the latest event inside the phrase, every lane of a chord), the origin high above the player's own neck and clamped inside their rails (`strike_origin` — never over a duet partner's neck), the chain jagged by the arc's own `point` hash and pinned exactly on the fret at both ends (`chain_point`), the whole thing a function of age (`strike_phase`: build 50 ms, flare 40 ms, fade 260 ms — 0.35 s, inside the impulse's 0.45). Pools of five bolts per player with hull children and one point light per player; per frame transforms, visibility and a light's intensity. The full-screen flash fires only without the 3D stage now. *Verified: 1975 tests (+5), gate green; seven mutations — first-not-last note, origin unclamped, chain ends unpinned, impact before landing, calm flaring, a fade past the impulse (a compile-time pin), no restart on a second phrase — all caught; frames on "Heroes" at 20.02/20.1/20.25 s with `reduced_flashing` off and on looked at and luma-read (fret 220/215/231 at impact, `screen 0.00` in the impulse's report); uncapped with the window raised 6.48–6.56 ms a frame against 6.50–6.55 on the previous binary.*
...
## Product website (DONE 2026-09-26)

- [x] **beatbyte.celox.io** — generated by the product-page kit (`/Users/martin/claude/_templates/apps/product-page`) from `website/site.json`: five languages, downloads for four targets with SHA-256, changelog and licence dialogs, JSON-LD, `index.md`/`llms.txt`, agent catalog. `beat-byte.celox.io` 301s with the path kept; one Let's Encrypt certificate for both. A VPS timer republishes the newest game release every 15 minutes — the kit gained `release_tag` for it, because GitHub's "latest" release here is `models-v1`. README links it at the top and under Installation; the repo homepage points to it. Open: a "Website" link on the in-game About screen; the page shows v0.18.18 until the next release is tagged.

## Live menus (DONE 2026-09-26, v0.18.51)

- [x] **Every screen shows what is true while it is open** — an audit of all menu screens found the roster built once (new/renamed/chosen players only after a revisit), the browser's LYRICS column blind to L and K until a restart, the song info page blind to the librarian's rewrites, and the achievements screen unordered against the quiet credit on the same entry. Fixed: the roster rebuilds on `Players`/`PlayHistory` changing with the playing player on top (`display_order`) and the cursor following its player (`cursor_after_rebuild`), plus the typing hint in the footer; a finished lookup or alignment rescans the library (the browser already rebuilds on that); the song info page re-reads `song.json` on a newer modification time (one `stat` a second) and keeps the scroll; the screen spawn is ordered after `credit_quietly`. Every other screen was already live (settings, controls, calibration, input test, join, stats, results, about, pause, editor). Open: `McQueue` keeps indices into the library, so a rescan while songs are queued can queue different songs. *Verified: 1981 tests (+4), gate green; seven mutations (no rebuild, playing player not first, old rows kept, cursor not following, document never re-read, old screen kept) all caught.*

## Performance on the M1 Pro and the 2015 MacBook Pro (IN PROGRESS 2026-09-25 — docs/perf/)

Goal: 60 fps under vsync on both machines, no missed frame over a whole song, also after 15 minutes of load. Commission in phases; after Phase 2 the table goes to the user and nothing is rebuilt without a go.

- [x] P0 **The bench** — `BEATBYTE_BENCH=<scenario>` on top of the autopilot (`bench.rs`): wall frame time from 5 s of song time on, frames over 16.7 ms, main-world CPU, GPU (opt-in, `null` where the device has no timestamps — the M1 Pro reports zeros), counts, song start, GPU name, resolution, settings, clock jumps; windowed 1280x720 by default (full screen only on request — it takes a user's display), cost probes (`BEATBYTE_PROBE`). `tools/bench.sh` (scenarios normal/dense/hype/soak, three runs, machine, commit, load; refuses a busy or locked machine and uncounted failed runs), `tools/bench-compare.py` (Δ %, `--fail-over`). The autopilot's teleport check measured against the clamped virtual delta and failed every one-second stall; now wall time. *Verified: 1977 tests (+2), gate green; the pure statistics (nearest-rank percentiles, the over-budget count) tested.*
