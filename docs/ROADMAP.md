...
- [ ] G33 **Section loop (optimization plan P1, second half).** Loop bounds set from the pause menu, seek back on reaching the end, the section's notes re-judged. Needs a session rewind in core (reset note states in a time range) and a music seek + clock re-anchor + note-entity refresh in gameplay. dep: G32.
