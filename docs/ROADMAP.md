...
- [ ] G54 **The bar rows have no rhythm.** Found in the S3 screenshots, not on the plan: in `plot::spawn_bar_plot` the rows of TECHNIQUE and SONGS are 21, 21, 21, 29, 37, 45 px apart while DIFFICULTY's are all 21, and in the loose rows the bar sits ~8 px BELOW its own label rather than beside it. DIFFICULTY's labels are single words of at most 8 characters; every loose row's label is 10 characters or more, which is the correlation — but the cause is **not** what it looks like: widening the 96-px label column to 160 (measured: the track really did move 64 px right) changed the rhythm not at all, and neither did replacing the spaces with U+00A0. So it is neither the column width nor a word wrap at a space. ⚠️ Three of my probes were invalid on the way there — a single anti-aliased column sampled as if it were a measurement, a mean luma read as an identity (the same pixels moved sideways give the same mean), and a colour filter that caught the label text while claiming to find the track. Measure the node tree, not the picture. *Verify: every row of a bar plot the same height whatever its label, with the bar beside the label; long song titles still legible.*
...
- [ ] **K — The classic programme.** A third chart twin beside the plain one and `[GS]`, working name **`[CL]`**, built from the measured rules of the early guitar games (`local/gh-klassik-recherche-2026-09-24.md`; only numbers and rules come in, never a chart or a note of anybody else's music, and never the marks themselves). One ingredient at a time, each switchable on its own, each decided by the user's BLIND TEST rather than by a metric — that is the G20 lesson, where the measurements improved and the playing got worse. `beatbyte-chart::classic` holds them and re-flags an EXISTING chart, so every test compares exactly one variable.
  ...
  - [ ] K2 **Strum tolerance.** Their engine gives a strum ~60 ms to find its fret: press shortly after and it still counts. **Measured first, read-only, from the recorded sessions: on Hard, 18 of 139 overstrums (13 %) show the pattern the buffer exists to rescue** — a fret pressed within 60 ms after the strum AND a note missed within ±100 ms of it. On Medium, 0 of 10. ⚠️ But that rests on FOUR sessions out of 144: detail level `Results` does not record the individual presses and 138 of the honest sessions are at it, so the question is unanswerable for 97 % of the evidence. A counter-check says the zero in the stricter reading is real and not a broken query — the nearest successful note after an overstrum is at least 160 ms away, median 551. Raise SETTINGS → TELEMETRY to `actions` and the next few sessions will settle it.
  - [ ] K3 **Hard the way they wrote it.** Their Hard has 11–15 % hammer-ons, and they come from fast pairs and triplets our Hard cannot contain — the fixed 0.16 s minimum spacing and a grid without triplets. K1 leaves our Hard near 0 %; this is what brings it back, honestly.
  - [ ] K4 **Medium: density and beat-faithfulness.** Theirs sits at 2.0–2.45 notes/s with 57–63 % of notes on a beat; ours at 1.54 and 50 %.
  - [ ] K5 **Chords.** Theirs are ~30 % of notes on Hard and Expert; ours are 2–4 %, and a chord invented from a monophonic reading would be a guess. This waits for polyphonic recognition (C3) and is deliberately last.
