...
- [ ] G48 **The mute badge.** Deferred twice without being filed: the Player Analytics spec puts "Mute badge / other UI" out of scope as "a separate task", and G45 records it as deferred — but nothing carried it here, so it existed only as an aside in two documents. Scope is whatever was meant by it; the point of this line is that it is no longer invisible. *Verify: the spec's aside is either built or closed on purpose.*
