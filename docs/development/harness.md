...
| `BEATBYTE_PROBE` | comma list: `msaa`, `bloom`, `shadows`, `lights`, `hidpi` | Bench runs only: takes one cost out of the frame so its price can be read off the difference — MSAA off, bloom removed from both cameras, the directional shadow map off, every point and spot light removed, one pixel per point (windowed only; in full screen macOS keeps the native resolution). The result names the probes. |
