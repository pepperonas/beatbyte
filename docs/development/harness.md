...
| `BEATBYTE_AUTOPILOT_TWINS` | set | Lets an autopilot run make `[Guitar Study]` twins for the songs it imports (the drop drill). Off by default under the autopilot because a stem separation saturates the machine for two minutes and the clock-teleport rule false-fails under that load; a player's own imports get their twin whenever SETTINGS → GUITAR STUDY TWINS is on. |
